...
I ablated MFCC, FBank, SDC, PLP and LPCC under identical UBM-MAP conditions. LPCC 13 + Δ + ΔΔ (LPC order 12, per-utterance CMN) won on both EER and min-DCF: 4.21 % EER for MFCC, 3.33 % for LPCC. The higher-dimensional alternatives (FBank 120-d, SDC 104-d) over-parameterised the GMM-32 backbone for this dataset size; PLP's Bark filterbank with cube-root compression produced fold-2 collapse. LPC order 12 had a 2.5 pp moat over its neighbours (orders 8, 10, 14, 16, 20 all worse by at least that much), which matches the `fs/1000 + 2` rule for 16 kHz speech.

### 3.2 Classifier: UBM-MAP with tied covariance

The backbone is the standard UBM-MAP setup. A 32-component GMM is trained on all non-target training frames to get the UBM, then the target model is produced by MAP-adapting only the UBM means with relevance factor r = 16. The frame-level score is the log-likelihood ratio between the target model and the UBM, summed over frames. I rejected UBM-64 in E010 (over-fits), CMVN normalisation in E012 (regresses by 2 pp; the UBM's covariance already absorbs the per-utterance scale), and confirmed that r ∈ {4, 8, 16} is a flat plateau on both diagonal and tied covariance (E013, E044).
...
At training time I apply two augmentations on top of the LPCC frames. The first is pitch shift in the range ±1–2 semitones (E025), which trains the model to ignore F0 while preserving formant ratios. This augmentation works specifically for LPCC; the same shift hurt MFCC in E014 because the Mel filterbank already smooths pitch information. The second is codec simulation (E052): each utterance is downsampled to 8 kHz and resampled back, which destroys frequencies above 4 kHz, where many of the discriminative formants live.
